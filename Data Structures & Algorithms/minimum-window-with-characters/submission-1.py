...
class Solution:
    def minWindow(self, s: str, t: str) -> str:
        ...
        while r < n:
            ...
            #is what we are adding to the window something we need?
            if counts.get(cr, 0) < target_counts.get(cr, 0):
                have += 1 
            counts[cr] = counts.get(cr, 0) + 1

            #isValid = all(target_counts[k] <= counts[k] for k in target_counts.keys())
            ...
            if isValid:
                ...
                while s[l] not in target_counts or counts[s[l]] > target_counts[s[l]]:
                    if s[l] in target_counts and counts.get(s[l], 0) == target_counts.get(s[l]):
                        # we needed that val
                        have -= 1
                    counts[s[l]] -= 1
                    if counts[s[l]] == 0:
